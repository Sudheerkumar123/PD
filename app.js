...
app.controller('myCtrl', function($scope) {
    ...
    $scope.addInfo=function(row){
        ...
        if(row.details.length>0)
        {
          ...
          {
            //{'name':'Sudheer Kumar','firstName':'Sudheer','lastName':'Kumar','address':'Hyderabad, Telangana','contactNumber':'8806264108'}
            $scope.details.push({'name':row.details[0].name,'firstName':row.details[0].firstName,'lastName':row.details[0].lastName,'address':row.details[0].address,'contactNumber':row.details[0].contactNumber});
